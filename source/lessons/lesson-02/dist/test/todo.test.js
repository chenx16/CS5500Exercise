...
describe('TodoList Model', () => {
    ...
    describe('Core Functionality - Adding Tasks', () => {
        ...
        describe('Error Handling', () => {
            ...
            test('should reject null task titles', () => {
                expect(() => todoList.addTask(null)).toThrow('Task title cannot be empty');
            });
            test('should reject undefined task titles', () => {
                expect(() => todoList.addTask(undefined)).toThrow('Task title cannot be empty');
            });
        });
    });
    describe('Core Functionality - Marking Tasks Complete', () => {
        ...
        describe('Error Handling', () => {
            ...
            test('should throw error for null task ID', () => {
                expect(() => todoList.completeTask(null)).toThrow('Task ID cannot be null or undefined');
            });
            test('should throw error for undefined task ID', () => {
                expect(() => todoList.completeTask(undefined)).toThrow('Task ID cannot be null or undefined');
            });
        });
    });
    describe('Core Functionality - Viewing Tasks', () => {
        ...
        describe('Edge Cases', () => {
            ...
            test('should return a copy of tasks array to prevent external modification', () => {
                ...
                const tasks = todoList.getTasks();
                tasks.push({}); // Try to modify returned array
    ...
    describe('Core Functionality - Deleting Tasks', () => {
        ...
        describe('Error Handling', () => {
            ...
            test('should throw error for null task ID', () => {
                expect(() => todoList.deleteTask(null)).toThrow('Task ID cannot be null or undefined');
            });
            test('should throw error for undefined task ID', () => {
                expect(() => todoList.deleteTask(undefined)).toThrow('Task ID cannot be null or undefined');
            });
        });
    });
    describe('Organization Features - Priority Levels', () => {
        ...
        describe('Happy Path', () => {
            test('should add task with priority', () => {
                ...
                const taskPriority = todo_1.Priority.HIGH;
                ...
                const expectedPriority = todo_1.Priority.HIGH; // Task should retain the assigned priority
                // Verification: Test that the user story requirement is met
            ...
            test('should default to medium priority when not specified', () => {
                ...
                expect(task.priority).toBe(todo_1.Priority.MEDIUM);
            });
            test('should update task priority', () => {
                const task = todoList.addTask('Changeable priority');
                todoList.updateTaskPriority(task.id, todo_1.Priority.HIGH);
                expect(task.priority).toBe(todo_1.Priority.HIGH);
            });
            test('should get tasks sorted by priority', () => {
                const lowTask = todoList.addTask('Low priority', { priority: todo_1.Priority.LOW });
                const highTask = todoList.addTask('High priority', { priority: todo_1.Priority.HIGH });
                const mediumTask = todoList.addTask('Medium priority', { priority: todo_1.Priority.MEDIUM });
        ...
        describe('Edge Cases', () => {
            test('should handle all priority levels', () => {
                const highTask = todoList.addTask('High', { priority: todo_1.Priority.HIGH });
                const mediumTask = todoList.addTask('Medium', { priority: todo_1.Priority.MEDIUM });
                const lowTask = todoList.addTask('Low', { priority: todo_1.Priority.LOW });
                expect(highTask.priority).toBe(todo_1.Priority.HIGH);
                expect(mediumTask.priority).toBe(todo_1.Priority.MEDIUM);
                expect(lowTask.priority).toBe(todo_1.Priority.LOW);
            });
            test('should maintain priority after completion', () => {
                const task = todoList.addTask('Important task', { priority: todo_1.Priority.HIGH });
                todoList.completeTask(task.id);
                expect(task.priority).toBe(todo_1.Priority.HIGH);
            });
        });
        describe('Error Handling', () => {
            test('should throw error when updating priority of non-existent task', () => {
                expect(() => todoList.updateTaskPriority('invalid-id', todo_1.Priority.HIGH))
                    .toThrow('Task not found');
            });
            test('should throw error for invalid priority value', () => {
                const task = todoList.addTask('Test task');
                expect(() => todoList.updateTaskPriority(task.id, 'INVALID'))
    ...
    describe('Productivity Features - Search', () => {
        ...
        describe('Error Handling', () => {
            test('should handle null search term', () => {
                todoList.addTask('Task 1');
                expect(() => todoList.searchTasks(null)).toThrow('Search term cannot be null or undefined');
            });
            test('should handle undefined search term', () => {
                todoList.addTask('Task 1');
                expect(() => todoList.searchTasks(undefined)).toThrow('Search term cannot be null or undefined');
            });
        });
    });
    describe('Data Persistence', () => {
        ...
        describe('Happy Path', () => {
            test('should serialize todo list to JSON', () => {
                todoList.addTask('Task 1', { priority: todo_1.Priority.HIGH });
                todoList.addTask('Task 2', { category: 'Work' });
            ...
            test('should deserialize todo list from JSON', () => {
                ...
                todoList.completeTask(task2.id);
                const json = todoList.toJSON();
                const newTodoList = todo_1.TodoList.fromJSON(json);
                expect(newTodoList.getTasks()).toHaveLength(2);
                expect(newTodoList.getTasks()[0].title).toBe('Task 1');
                expect(newTodoList.getTasks()[0].priority).toBe(todo_1.Priority.HIGH);
            ...
            test('should maintain task relationships after serialization', () => {
                ...
                expect(restoredTask.priority).toBe(todo_1.Priority.HIGH);
        ...
        describe('Edge Cases', () => {
            ...
            test('should handle large todo lists', () => {
                for (let i = 0; i < 100; i++) {
                    todoList.addTask(`Task ${i}`, {
                        priority: i % 3 === 0 ? todo_1.Priority.HIGH : todo_1.Priority.MEDIUM,
        ...
        describe('Error Handling', () => {
            test('should throw error for invalid JSON', () => {
                expect(() => todo_1.TodoList.fromJSON('invalid json')).toThrow('Invalid JSON format');
            });
            test('should throw error for null JSON', () => {
                expect(() => todo_1.TodoList.fromJSON(null)).toThrow('JSON string cannot be null or undefined');
            });
            test('should throw error for malformed todo data', () => {
                const invalidJson = JSON.stringify({ tasks: [{ invalidField: 'value' }] });
                expect(() => todo_1.TodoList.fromJSON(invalidJson)).toThrow('Invalid todo data format');
            });
        });
    });
});
