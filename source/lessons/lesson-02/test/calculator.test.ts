...
describe('Calculator', () => {
  ...
  describe('Unary operators', () => {
    ...
    test('should handle fallback unary minus (double minus)', () => {
      const result = calculator.calculate(['-', '-', '2']);
      expect(result.result).toBe(2);
      expect(result.error).toBeUndefined();
    });

    test('should handle fallback unary plus (double plus)', () => {
      const result = calculator.calculate(['+', '+', '2']);
      expect(result.result).toBe(2);
      expect(result.error).toBeUndefined();
    });
  ...
  describe('Error handling', () => {
    ...
    test('should throw error for unexpected end of expression (empty token)', () => {
      const result = calculator.calculate(['']);
      expect(result.error).toBe('Unexpected end of expression');
      expect(result.result).toBeUndefined();
    });

    test('should throw error for completely invalid token', () => {
      const result = calculator.calculate(['foo']);
      expect(result.error).toBe('Invalid token: foo');
      expect(result.result).toBeUndefined();
    });
  });
